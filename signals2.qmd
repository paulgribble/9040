...
ymin = min(psd_p[freqs_p > 1].min(), psd_w[freqs_w > 1].min()) * 0.5
ymax = max(psd_p[1:].max(), psd_w[1:].max()) * 2
...
The main parameter you choose is the **time-bandwidth product** (commonly called NW). A typical value is NW = 4, which gives 2×NW − 1 = 7 usable tapers. Higher NW means more tapers (lower variance) but broader frequency smoothing (lower resolution).

SciPy includes DPSS windows via [`scipy.signal.windows.dpss()`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.windows.dpss.html), so no extra packages are needed.
...
# Multitaper PSD — uses the full 2-second signal
...
# Average with eigenvalue weighting and normalize to PSD units
...
ax[1].plot(freqs_welch, psd_welch, linewidth=2, label=f'Welch PSD (nperseg=250, {fs/250:.0f} Hz resolution)')
ax[1].plot(freqs_mt, psd_mt, linewidth=2, label=f'Multitaper PSD (NW={NW}, {K} tapers)')
ax[1].set_xlim([0, 40])
...
### Using MNE-Python for multitaper (recommended in practice)

The step-by-step code above is useful for understanding what's happening under the hood, but in practice you don't need to do it by hand. The [MNE-Python](https://mne.tools/) package (install with `pip install mne`) provides a one-liner that handles the DPSS computation, tapering, and normalization for you:

```{python}
from mne.time_frequency import psd_array_multitaper

psd_mne, freqs_mne = psd_array_multitaper(yn, sfreq=fs, bandwidth=2.0, 
                                            normalization='full', verbose=False)
```

The `bandwidth` parameter (in Hz) controls the frequency smoothing---it plays the same role as the time-bandwidth product NW (specifically, bandwidth = 2 × NW / T, where T is the signal duration). Setting `normalization='full'` ensures the output is in the same V²/Hz units as `scipy.signal.welch()`, so you can compare them directly.

MNE is the de facto standard package for EEG and MEG analysis in Python, so if you end up doing that kind of work, you will likely already have it installed.
