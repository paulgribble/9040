...
# Part 1: Why Plaintext? Markdown and Pandoc

## What Is a Scientific Document
...
## Markdown: The Simplest Plaintext Markup
...
### Pandoc-Flavored Markdown Extensions
...
## Exercise 1: Your First Pandoc Markdown Document

### Setup

Make sure you have Pandoc and a LaTeX distribution installed (see above).

Download the [files/plaintext-exercises.tgz](files/plaintext-exercises.tgz) archive and then unpack it:

```{bash}
$ curl -O https://www.gribblelab.org/9040/files/plaintext-exercises.tgz
```
...
# Part 2: LaTeX for Scientific Documents (60 minutes)

## 2.1 — What Is LaTeX and Why Does It Exist? (8 min)
...
| Paper with complex equations | LaTeX directly |
| Paper with precise figure placement | LaTeX directly |
...
## 2.2 — LaTeX Document Structure (10 min)
...
## 2.5 — Breakout Exercise 2: Building a LaTeX Document (15 min)

### Option A: Using Overleaf (Recommended for First-Timers)

1. Go to <https://www.overleaf.com/> and log in.
2. Create a new blank project.
3. Start with the minimal template from Section 2.2 above.
...
## 2.6 — Overleaf: Collaborative LaTeX in the Browser (5 min)

[Overleaf](https://www.overleaf.com/) deserves special mention because it addresses the most common objection to LaTeX: collaboration.
...
- [CoCalc](https://cocalc.com/) — includes LaTeX, Jupyter, and a terminal
- [Papeeria](https://papeeria.com/) — lightweight alternative to Overleaf

---

## 2.7 — Putting It All Together: Choosing Your Workflow (4 min)
...
## 2.8 — Breakout Exercise 3: The Full Pipeline (10 min)
...
2. **Compile to Word** with Pandoc. Open in Word or LibreOffice. Confirm it looks reasonable.
3. **Compile to LaTeX** with `pandoc paper.md -o paper.tex`. Open the `.tex` file and look at what Pandoc generated.
4. **Edit the `.tex` file**: adjust figure placement, tweak table formatting, or add a LaTeX-specific feature like `\pagebreak`.
5. **Compile the edited `.tex` to PDF**: `latexmk -pdf paper.tex`
...
## 2.9 — Quick Reference Card and Resources (2 min)
...
1. **Explain** why plaintext authoring is valuable for reproducibility, portability, and archival.
2. **Write** a scientific document in Markdown with sections, citations, equations, and figures.
3. **Use Pandoc** to convert that document to PDF, Word, or HTML.
4. **Write** a LaTeX document directly, with equations, figures, tables, and a bibliography.
5. **Use Overleaf** for collaborative LaTeX editing.
6. **Choose** the right workflow for a given writing task.
7. **Convert** between formats as needed — including generating a `.docx` when a journal requires it.
