...
The Thomson [multitaper method](https://en.wikipedia.org/wiki/Multitaper) is a way to overcome these limitations. We won't go over the details here, you can read about it in the Wikipedia article linked above. 

In Python there is a package called [`spectrum`](https://pyspectrum.readthedocs.io/en/latest/index.html) (install using `pip install spectrum`) that contains functions for doing [multitapering](https://pyspectrum.readthedocs.io/en/latest/ref_mtm.html) PSD estimates. For the purposes of this course I don't mind which you use, Welch or something else.
...
-   *Butterworth Filters* have frequency responses which are maximally
    ...
    For example in my work I use them exlusively for filtering
...
-   *Bessel Filters* have a phase-shift which is linear with frequency
    ...
    filters preserve the shape of the signal quite well. The roll-off is
    monotonic and approaches the same slope as the Butterworth and
...
is only capable of using $0.0025/10*4096=10$ (ten) unique values to
